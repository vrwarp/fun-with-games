...
| **sim-core**       | `src/sim/world.ts`, `types.ts`, `config.ts`, `step.ts`, `rng.ts`, `presets.ts`, `track.ts` | `World`, `WorldSnapshot`, `SimConfig`, `StepContext`, `modeConfig`                                | shared                      | State container, tick pipeline, snapshots, mode presets. **High-traffic — coordinate.**                                                                                          |
