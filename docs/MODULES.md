...
| ------------------ | --------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **shared**         | `src/shared/`                                                                                             | `math.ts`, `emitter.ts`, `logger.ts`, `manifest.ts`, `modes.ts`                                                  | —                           | Utilities + game-mode metadata. Changes here touch everyone; prefer adding over modifying.                                                                                                                                    |
| **sim-core**       | `src/sim/world.ts`, `types.ts`, `config.ts`, `step.ts`, `rng.ts`, `presets.ts`, `track.ts`, `controls.ts` | `World`, `WorldSnapshot`, `SimConfig`, `StepContext`, `modeConfig`, `axesForDirection`                           | shared                      | State container, tick pipeline, snapshots, mode presets, and the input contract bots and players share. **High-traffic — coordinate.**                                                                                        |
| **sim-systems**    | `src/sim/systems/`                                                                                        | one module per system                                                                                            | shared, sim-core types      | Movement, phases, combat, projectiles, tag, ball, zones, items, bots, pickups, effects, vehicles, racing. **Best place for parallel gameplay work** — new systems are new files.                                              |
...
| **render**         | `src/render/`                                                                                             | `Renderer`, `EntityViews`, `KitViews`, `TrackView`, `CarFinishes`, `KeyboardInput`, `TouchInput`, `TouchButtons` | shared, sim types, net view | Babylon scene, meshes, materials and lighting, kit visuals (zones/ball/projectiles/items), circuits, follow-camera, input (keyboard **and** touch **and** action buttons), device APIs. See [`RENDERING.md`](./RENDERING.md). |
| **ui**             | `src/ui/`                                                                                                 | `Hud`, `Announcer`, `Lobby`, `Credits`, `styles.css`                                                             | shared, net view            | DOM overlay: scoreboard, phase banner, timers, toasts, mode picker. Independent of `render` — **parallel-friendly**.                                                                                                          |
...
| Add               | Where                                                | Touches                                            |
| ----------------- | ---------------------------------------------------- | -------------------------------------------------- |
| Game mode         | `src/shared/modes.ts` + `src/sim/presets.ts`         | two appends; auto-covered by `presets.test.ts`     |
| Circuit           | control points in `src/sim/presets.ts`               | sim-core only; covered by `track.test.ts`          |
| Gameplay mechanic | `src/sim/systems/<name>.ts`                          | sim-systems + one line in `world.ts`               |
| Player ability    | read `input.buttons` in a system                     | sim only — buttons already on wire/keyboard/touch  |
| Timed status      | `addEffect(player, 'id', untilTick)`                 | sim only — effects are snapshotted generically     |
...
| Material or light | `src/render/` — see [`RENDERING.md`](./RENDERING.md) | render only                                        |
